...
func gmailValidation(email string) bool {
	_, err := mail.ParseAddress(email)
	return (err == nil) && (strings.HasSuffix(email, "@gmail.com"))
}
...
func CheckCreateAccount(b *bot.Bot, message *tgbotapi.Message) error {
	...
	if err != nil {
		infoLog.Printf("У '%v' отсутствует сохраненный gmail.\n", message.Chat.UserName)
		...
		if gmailValidation(strings.TrimSpace(message.Text)) {
			...
			if err != nil {
				...
				infoLog.Printf("Сохранение '%v' корректного gmail: '%v'.\n", message.Chat.UserName, message.Text)
			}
		} else {
			infoLog.Printf("Введенное '%v' значение '%v' не является gmail.\n", message.Chat.UserName, message.Text)
	...
	if err != nil {
		...
		if _, err := b.BotAPI.Send(msg); err != nil {
			...
			infoLog.Printf("Проверка наличия gmail и таблицы '%v' пройдена успешно.\n", message.Chat.UserName)
		}
	}
	return nil
}
